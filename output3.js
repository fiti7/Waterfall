...
color: 'transparent',
    showInLegend: false, data: [28, 1042, 1042, 1042, 1042, 1043, 1043, 2442, 2480, 5764, 5767, 5768, 5768, 5772, 5772, 5772, 5772, 5865, 6008, 6138, 6496, 6923, 7328, 7733, 7742, 7893, 8096, 8115, 8247, 8625, 8934, 9170, 9232, 9303, 9362, 9523, 9586, 9656, 9680, 9715, 9877, 9941, 9991, 10010, 10033, 10067, 10344, 10363, 10396, 10396, 10419, 11528, 5, 625, 5835, 5835, 6192, 6537, 6567, 6923, 6927, 7314, 7657, 7657, 7657, 7658, 7658, 9446, 9450, 9960, 9961, 9961, 9961, 9961, 12670, 14166, 14169]}
        		...
        		function makeline(chart) { // on complete
            var test = 100;
            var element = document.getElementById('media');
            style = window.getComputedStyle(element);
           // var scrollPercent = .99 * (element.scrollLeft) / (element.scrollWidth-$(chart.container).width()); 	
            var scrollPercent = (.97 * (element.scrollLeft) / (element.scrollWidth));
           
            //0 to 4620 * 100 / 4620 - 1403
            
            // var myheight = document.getElementById("container").style.height.slice(0,-2);
           //x and y are reversed
            var mywidth = $(chart.container).width(); 
            var myheight =  scrollPercent * $(chart.container).height() ;
           
            console.log(scrollPercent);
            console.log("left" + element.scrollLeft); //0
            console.log("width" + element.scrollWidth);   //4620
            console.log("chartwidth" + $(chart.container).width()); //1500
            console.log("chartheight" + $(chart.container).height()); //2000
            chart.renderer.rect(myheight, 0, 2, mywidth*1.4)
                .attr({
                    'stroke-width': 2,
                    stroke: 'red',
                })
