...
# remain packets delta- Blue
# client time - system_delta- Dark blue
# MS_FIRST_PAINT_MSEC - redline
# DOM_INTERACTIVE_MSEC - greenline
...
def createoutputs(inputarray, maxseq, linearray):
    ...
    for num in range(maxseq):
        ...
        var starttime = " + str(starttimes[num]) + ";\
        var endtime = " + str(endtimes[num]) + ";\
        ...
        var startPercent = (starttime / " + str( endtimes[-1] )+ ");\
        var endPercent = (endtime / " + str( endtimes[-1] )+ ");\
...
if not os.path.exists("TransData.dat"):
    dat = glob.glob("*.dat")
    if (len(dat)!= 0):
        data = main(dat[-1])
