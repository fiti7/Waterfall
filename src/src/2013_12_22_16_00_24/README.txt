...
The python file will ask the user's default browser* to open the waterfall.html file. 
...
The first is by email. Mail.java logs into a gmail account (put your own info in), zips the relevant files and sends them back to myself. 
...
First it chooses which images to use. First it orders them by filename (elapsed time). 
...
The python file will ask the user's default browser (currently only chrome works) to open the waterfall.html file.
