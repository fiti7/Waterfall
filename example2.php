...
color: 'transparent',
    showInLegend: false, data: [28, 1042, 1042, 1042, 1042, 1043, 1043, 2442, 2480, 5764, 5767, 5768, 5768, 5772, 5772, 5772, 5772, 5865, 6008, 6138, 6496, 6923, 7328, 7733, 7742, 7893, 8096, 8115, 8247, 8625, 8934, 9170, 9232, 9303, 9362, 9523, 9586, 9656, 9680, 9715, 9877, 9941, 9991, 10010, 10033, 10067, 10344, 10363, 10396, 10396, 10419, 11528, 5, 625, 5835, 5835, 6192, 6537, 6567, 6923, 6927, 7314, 7657, 7657, 7657, 7658, 7658, 9446, 9450, 9960, 9961, 9961, 9961, 9961, 12670, 14166, 14169]}
     ...
     //grabs from "2000px" sample- have to slice to get rid of px
   //var mywidth = document.getElementById("container").style.min.width.slice(0,-2);
    ...
    var scrollPercent = .99 * element.scrollLeft / (element.scrollWidth-element.clientWidth); 	
     ...
     var myheight = document.getElementById("container").style.height.slice(0,-2);
        chart.renderer.rect(scrollPercent, 0, 2, myheight, 5)
            .attr({
                'stroke-width': 2,
                stroke: 'red',
                fill: 'yellow',
...
<!--
    ...
    var refreshrate = 100;
	    ...
	    setInterval(function render(){
		    ...
		    myline = renderer.rect(scrollPercent * chart.width,0,1, chart.height)
		            .attr({
		                stroke: 'red',
		                zIndex: 3
		            })
	...
	**/	
	$(function () {
				var chart = $('#container').highcharts();
				var renderer = $('#container').chart.Renderer( $('#container')[0], 
        chart.width,
        chart.height
    );
				myline = renderer.rect(100,100,1,1000).attr({
                'stroke-width': 5,
                stroke: 'red',
                zIndex: 3})
					.add();
...
/**					
    			...
    			style = window.getComputedStyle(element);
    			var scrollPercent = .99 * element.scrollLeft / (element.scrollWidth-element.clientWidth); 	
    			myline = renderer.rect(scrollPercent * cvs.width,0,1, cvs.height) .attr({
                'stroke-width': 2,
                stroke: 'red',
                fill: 'yellow',
                zIndex: 3}).add();
