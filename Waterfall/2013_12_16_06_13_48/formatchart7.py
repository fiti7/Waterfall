...
elements = ["START_MSEC", "DNS_LOOKUP_MSEC", "CONNECT_DELTA", "SSL_HANDSHAKE_DELTA",
             ...
             "PAGE_SEQ", "RECORD_SEQ", "CONN_STRING_TEXT", "OBJECT_TEXT"]
...
def formatdata(data, elements):
    ...
    for datum in data:
        ...
        if newarray[-1] != 0 and newarray[-4] != 0:
...
def getmax(inputarray):
    ...
    #get the number of transactions/pages
    i = 0
    while i < (len(inputarray[-4]) - 1):
        if int(inputarray[-4][i]) > maxseq:
            maxseq = int(inputarray[-4][i])
        i+= 1
...
def splitTransactions(inputarray, maxseq):
    ...
    while i < (len(inputarray[-4]) - 1):
        ...
        for k in range(len(inputarray)):
            miniarray[int(inputarray[-4][i]) - 1][k].append(inputarray[k][i]) 
        i+= 1
...
def reducetext(inputarrayA, inputarrayB):
    ...
    while i < (len(inputarrayA) - 1):
        if(type(inputarrayA[i]) == str and type(inputarrayB[i]) == str):
            inputarrayA[i] = (inputarrayB[i] + inputarray[i])[0:40]
        i+=1
...
def createoutputs(inputarray, maxseq, linearray):
    ...
    for num in range(maxseq):
        ...
        print("var current = "+inputarray[num][-4][-1]+";\
...
#       20]                          // default blue       
        ...
        myout.write(inputarray[num][-4][-1])
        for item in linearray:
            if(item[-4] != 0):
                    print("\
            if ( current == "+str(item[-4]) +"){\n\
            chart.renderer.rect(\
...
#print(maxseq)
reducetext(dataarray[-1], dataarray[-2])
output = splitTransactions(dataarray, maxseq)
#print(output) 
createoutputs(output, maxseq, linearray)
webbrowser.get('windows-default').open('http://localhost/Waterfall/' + sys.argv[0].split("\\")[-2] + '/linedbase4.php?')
